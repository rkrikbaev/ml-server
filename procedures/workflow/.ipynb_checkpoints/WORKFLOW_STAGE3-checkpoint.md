...
- `src/api/forecast/config.py` -> `load_model_config(model_id)`
- `src/api/forecast/provider.py` -> `sync_with_registry(model_id, selector)`
...
`load_model_config(model_id)` в `src/api/forecast/config.py`:
...
Вызывается в `src/api/forecast/model.py` -> `init_model(...)`, который строит адаптер по `model_type`:
...
- Файлы: `src/api/forecast/model.py` -> `init_model`
...
curl -sS -X POST http://localhost:8030/predict \
  ...
  -d '{"object_reference":"/root/FP/PROJECT/AKMOLA/@regions/does-not-exist","model_id":"nonexistent_model"}'
```

Далее опросить по `task_id`. Ожидание:

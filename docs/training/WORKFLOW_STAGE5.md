...
- `src/adapters/model.py` -> `init_model(...)`
- `src/adapters/inference.py` -> `predict(...)`
...
Важно: в текущей реализации `src/adapters/inference.py::predict(...)` ловит исключения внутри себя и обычно не пробрасывает их наружу.

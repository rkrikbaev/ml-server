# ✅ COMPLETE EXPLANATION: Training Pipeline
...
> Implementation note: `POST /train` does **not exist** and will **not be implemented**. Training is a manual engineer workflow in Jupyter notebooks. There is no HTTP trigger for training.
...
### Stage 1: Preparation + Pre-flight Checks
```
Input: Engineer opens notebook with object_reference, model_type, train_params
...
Output: readiness check passed before launching training
```
...
Process - Baseline (REQUIRED first):
├─ Build lag + time features (hour, day-of-week sin/cos)
├─ Fit Ridge/LinearRegression
└─ Log metrics as model_type=linear_baseline to MLflow
...
Note: Prophet/XGBoost must outperform baseline on ≥1 metric
...
### Step 6: Итоги и регистрация модели в MLflow
```
Input: run_id, metrics, trained model in MLflow

Process:
├─ Review logged metrics in MLflow UI
├─ Compare baseline vs Prophet/XGBoost metrics
├─ Assign MLflow alias (e.g. "Production") to winning run
└─ Confirm bundle/configuration/cache_config.json is correct

Output - Success:
  Run logged with mae, mape, rmse + model artifact + cache_config.json
  Model alias "Production" assigned to the winning run

Output - Failure:
  Run marked as FAILED in MLflow; no alias assigned

⏱️ Duration: < 1 min (engineer review)
```
...
**14:34:13** Engineer reviews metrics in MLflow UI
**14:34:14** Model alias "Production" assigned ✅
...
6. **Review and Register** - Engineer reviews metrics, assigns MLflow alias (e.g. "Production")

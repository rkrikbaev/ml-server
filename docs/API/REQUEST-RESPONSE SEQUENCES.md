...
1. Client calls `GET /predict/{model_id}` with optional `version_alias` and `object_ref` query params.
2. `model-server` enqueues async prediction and returns `202` with `task_id`.
3. Client polls `GET /tasks/{task_id}` until the task is ready.
4. For offline models (`model_id != none`), `model-server` resolves `run_id` from MLflow Registry using `version_alias` from query params.
  If `version_alias` is omitted, default alias from `MLFLOW_DEFAULT_ALIAS` is used (default value: `Production`).
...
- `jupyter-notebook-server`:
  ...
  - Logs runs/artifacts into MLflow and can call runtime API for validation (`GET /predict/{model_id}` + `GET /tasks/{task_id}`).
...
- `model_registry_cache` volume:
  - Runtime bundle cache in `/tmp/local_models_cache`.
...
```text
                        ...
                        +-------------+---------------+
                                      |
                         HTTP GET /predict/{model_id} | Notebook UI (browser)
                                      |
                                      v
+----------------------------------------------------------------------------+
...
|   | /tmp/local_models_cache |   store bundle + metadata               |
...
Client               model-server              task queue/result backend   MLflow Registry   local/mlruns   runtime cache
  |                       |                              |                   |                 |              |
  | GET /predict/{model_id}?version_alias=...&object_ref=...            |                 |
  |---------------------->| enqueue task                 |                   |                 |
  |<----------------------| 202 START {task_id}          |                   |                 |
  |                       |                              |                   |                 |
  |                       | check task status            |                   |                 |
  ...
  |                       |---------------------------------------------------------------------------------> |
  |                       | infer forecast                                                     |
  | GET /tasks/{task_id}  | return final task result                                           |
  |---------------------->|                                                                    |
...
User (DS/Analyst)      Jupyter Notebook Server      MLflow Registry/Artifacts      model-server
  ...
  |------------------------->| GET /predict/{model_id} + GET /tasks/{task_id} |            |
  |                          |------------------------------------------------------->|
  |                          |<-------------------------------------------------------|
...
- Registry selector resolution priority: `version_alias` -> `MLFLOW_DEFAULT_ALIAS` (default value: `Production`).
